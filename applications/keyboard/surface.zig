//! The Keyboard surface: the human door onto Keyboard — a header, a status line, and its actions, laid
//! out from the design tokens over the shared helper. Every colour is a resolved token.

const std = @import("std");
const surface = @import("../framework/surface.zig");
const app = @import("keyboard.zig");

pub const Row = surface.Row;

/// Renders the Keyboard surface into `buffer`.
pub fn render(buffer: []Row) []const Row {
    ...
    return surface.layout("Keyboard", "Ready", &actions, buffer);
}

const testing = std.testing;

test "the keyboard surface renders a header, a status line, and its actions" {
