//! The Camera surface: the human door onto the app's one domain — a header, a line of
//! its real state, and a row for each thing the person can do, laid out from the design
//! tokens over the shared surface helper.
//!
//! A person reaches Camera here; an agent reaches the same domain through its
...
const app = @import("camera.zig");

pub const Row = surface.Row;

/// Renders the Camera surface from the real store into `buffer`. `scratch` holds the
...
pub fn render(store: *const app.Store, scratch: []u8, buffer: []Row) []const Row {
    ...
    const summary = std.fmt.bufPrint(scratch, "{d} shots", .{store.shots}) catch "";
    return surface.layout("Camera", summary, &actions, buffer);
}
