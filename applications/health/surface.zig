//! The Health surface: the human door onto Health — a header, a status line, and its actions, laid out
...
const app = @import("health.zig");

pub const Row = surface.Row;

/// Renders the Health surface into `buffer`.
pub fn render(buffer: []Row) []const Row {
    ...
    return surface.layout("Health", "Ready", &actions, buffer);
}

const testing = std.testing;

test "the health surface renders a header, a status line, and its actions" {
