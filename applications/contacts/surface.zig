//! The Contacts surface: the human door onto the app's one domain — a header, a line of
//! its real state, and a row for each thing the person can do, laid out from the design
//! tokens over the shared surface helper.
//!
//! A person reaches Contacts here; an agent reaches the same domain through its
...
const app = @import("contacts.zig");

pub const Row = surface.Row;

/// Renders the Contacts surface from the real store into `buffer`. `scratch` holds the
...
pub fn render(store: *const app.Store, scratch: []u8, buffer: []Row) []const Row {
    ...
    const summary = std.fmt.bufPrint(scratch, "{d} contacts", .{store.count()}) catch "";
    return surface.layout("Contacts", summary, &actions, buffer);
}
