//! The Files domain: the documents a person and their agents browse and organize.
//!
...
/// The files store: its committed state and the record of which keyed operations have
...
pub const Store = struct {
    ...
    committed: std.ArrayListUnmanaged(Applied) = .empty,

    pub fn init(gpa: std.mem.Allocator) Store { return .{ .gpa = gpa }; }

    pub fn deinit(store: *Store) void {
        store.committed.deinit(store.gpa);
        store.* = undefined;
    }

    /// How many state changes have been committed.
    pub fn changes(store: Store) usize { return store.committed.items.len; }

    fn applyKeyed(store: *Store, key: u128, result: []const u8) DomainResult {
        if (priorResult(store.committed.items, key)) |prior| return .{ .ok = prior };
        store.committed.append(store.gpa, .{ .key = key, .result = result }) catch return .failed;
        return .{ .ok = result };
    }
    ...
    pub fn execute(context: *anyopaque, input: Input, actor: Actor, key: u128) DomainResult {
        ...
        if (std.mem.eql(u8, operation, "file.list")) return .{ .ok = "read" };
        if (std.mem.eql(u8, operation, "file.open")) return .{ .ok = "read" };
        if (std.mem.eql(u8, operation, "file.edit")) return store.applyKeyed(key, "edit");
        if (std.mem.eql(u8, operation, "file.move")) return store.applyKeyed(key, "move");
        if (std.mem.eql(u8, operation, "file.share")) return store.applyKeyed(key, "share");
        if (std.mem.eql(u8, operation, "file.delete")) return store.applyKeyed(key, "delete");
        return .failed;
    }
