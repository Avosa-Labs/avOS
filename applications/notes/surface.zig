//! The Notes surface: the human door onto Notes — a header, a status line, and its actions, laid out
...
const app = @import("notes.zig");

pub const Row = surface.Row;

/// Renders the Notes surface into `buffer`.
pub fn render(buffer: []Row) []const Row {
    ...
    return surface.layout("Notes", "Ready", &actions, buffer);
}

const testing = std.testing;

test "the notes surface renders a header, a status line, and its actions" {
