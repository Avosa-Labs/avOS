//! The Tasks surface: the human door onto Tasks — a header, a status line, and its actions, laid out
...
const app = @import("tasks.zig");

pub const Row = surface.Row;

/// Renders the Tasks surface into `buffer`.
pub fn render(buffer: []Row) []const Row {
    ...
    return surface.layout("Tasks", "Ready", &actions, buffer);
}

const testing = std.testing;

test "the tasks surface renders a header, a status line, and its actions" {
