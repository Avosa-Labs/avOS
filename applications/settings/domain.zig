//! The Settings domain: the device configuration a person and their agents adjust.
//!
...
/// The settings store: its committed state and the record of which keyed operations have
...
pub const Store = struct {
    ...
    pub fn execute(context: *anyopaque, operation: []const u8, actor: Actor, key: u128) DomainResult {
        ...
        if (std.mem.eql(u8, operation, "settings.read")) return .{ .ok = "read" };
        if (std.mem.eql(u8, operation, "settings.toggle")) return store.applyKeyed(key, "toggle");
        if (std.mem.eql(u8, operation, "settings.sensitive_change")) return store.applyKeyed(key, "sensitive_change");
        return .failed;
    }
