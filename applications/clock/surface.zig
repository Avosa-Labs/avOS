//! The Clock surface: the human door onto Clock — a header, a status line, and its actions, laid out
...
const app = @import("clock.zig");

pub const Row = surface.Row;

/// Renders the Clock surface into `buffer`.
pub fn render(buffer: []Row) []const Row {
    ...
    return surface.layout("Clock", "Ready", &actions, buffer);
}

const testing = std.testing;

test "the clock surface renders a header, a status line, and its actions" {
